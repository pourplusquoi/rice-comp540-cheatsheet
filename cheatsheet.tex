...
\begin{proof}
	...
	\end{align*}
	Define $\alpha^{(i)} = \frac{1}{m \lambda} \langle x^{(i)}, v \rangle$, then $v = \sum_{i=1}^m \alpha^{(i)} x^{(i)}$. Now project $x \in \mathbb{R}^d$ using $\phi: \mathbb{R}^d \rightarrow \mathbb{R}^D$ into a higher dimensional space. The covariance matrix $C$ of the transformed data is
	\begin{align}
	...
	\end{align}
	Substitute (4) and (5) into (3) getting
	\begin{align*}
