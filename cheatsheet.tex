...
\subsubsection{7.4.4 \ Parameters estimation}
\begin{itemize}
...
\begin{itemize}
	...
	\item $Action(s)$: possible actions from state $s$, no actions from terminal states.
...
\begin{itemize}
	...
	-- Repeat until convergence:\\
	\ \ \ \ -- For each state $s$:\\
	\ \ \ \ \ \ \ \ -- $V_\pi^{i}(s) = \sum_{s'} T(s, \pi(s), s') \left[ r(s, \pi(s), s') + V_\pi^{i-1}(s') \right]$
\end{itemize}

\subsubsection{9.1.4 \ Optimality}

\subsection{9.2 \ Model-based RL}
\subsection{9.3 \ Model-free RL}
\subsection{9.4 \ TD learning}
\subsection{9.5 \ Q learning}
