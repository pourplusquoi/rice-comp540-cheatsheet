...
\begin{itemize}
	\item Loss function: $J_l = \sum_{i=1}^m w_l^{(i)} I\left(h_l(x^{(i)}) \neq y^{(i)}\right)$
	\item Prediction: $h(x) = \mathrm{sign}\left(\sum_{l=1}^L \alpha_l h_l(x)\right)$
	...
	2. calculate error rate $\epsilon_l = \frac{\sum_{i=1}^m w_l^{(i)} I\left(h_l(x^{(i)}) \neq y^{(i)}\right)}{\sum_{i=1}^m w_l^{(i)}}$\\
...
\begin{itemize}
	\item Residuals are negative gradients (squared error loss):\\ $J = \sum_{i=1}^m \frac{1}{2}(y - h(x^{(i)}))^2 \Rightarrow \frac{\partial J}{\partial h(x^{(i)})} = h(x^{(i)}) - y^{(i)}$
\end{itemize}

\subsubsection{6.3.1 \ Gradient boosting for classification}
\begin{itemize}
	\item Loss function: $J = \frac{1}{m} \sum_{i=1}^m D_{\mathrm{KL}} (y^{(i)}, h(x^{(i)}))$
	\item Gradient boosting algorithm ($k$ classes):\\
	-- Start with an initial $h_1, ..., h_k$ for $x^{(1)}, ..., x^{(m)}$\\
	-- Repeat until convergence:\\
	\ \ \ \ -- Calculate matrix of gradients, fit each $h_{k'}^{\mathrm{add}}$ to the negative gradients, $h_{k'} \leftarrow h_{k'} + h_{k'}^{\mathrm{add}}$
\end{itemize}

\section{7 \ Probabilistic graphical models}
\subsection{7.1 \ Directed models -- Bayesian network}
\begin{itemize}
	\item $P(X) = \prod_i P(x_i | \mathrm{Parents}(x_i))$
...
\begin{itemize}
	...
	-- Sample $x_i$ from distribution $P(x_i | \mathrm{Parents}(x_i))$
\end{itemize}
...
\begin{itemize}
	...
	-- Iteratively visit each $x_i$ and sample a value for it based on $P(x_i | \mathrm{Neighbors}(x_i))$\\
...
\begin{itemize}
	...
	-- $\alpha_{t+1}(i) = b_i(e_{t+1})\sum_{j=1}^n \alpha_t(j) a_{ji}$, $1 \leq j \leq n$, $0 \leq t \leq T-1$
...
\begin{itemize}
	...
	-- $\delta_{t+1}(j) = b_j(e_{t+1}) \max_i \delta_t(i) a_{ij}$, $1 \leq j \leq n$, $0 \leq t \leq T-1$\\
	\item Time complexity: $O(n^2T)$
\end{itemize}
...
\begin{itemize}
	...
	\item Kullback-Leibler (KL) divergence: $D_{\mathrm{KL}} (p||q) = \sum_x p(x) \log{\frac{p(x)}{q(x)}}$
	\item Cross entropy: $H(p,q) = E_p[- \log q] = H(p) + D_{\mathrm{KL}}(p||q)$
\end{itemize}
